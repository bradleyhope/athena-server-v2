...
def apply_evolution(proposal: dict) -> dict:
    """
...
async def review_proposal(proposal_id: str, approval: ApprovalRequest):
    ...
    try:
        ...
        if proposal['status'] != 'proposed':
            raise HTTPException(
                status_code=400, 
                detail=f"Proposal is already {proposal['status']}, cannot review"
            )
...
async def apply_proposal(proposal_id: str):
    ...
    try:
        ...
        if proposal['status'] != 'approved':
            raise HTTPException(
                status_code=400, 
                detail=f"Proposal must be approved before applying. Current status: {proposal['status']}"
            )
        ...
        if result.get('success'):
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Failed to apply evolution: {result.get('error')}"
            )
    except HTTPException:
        raise
    except Exception as e:
...
async def trigger_evolution_engine():
    ...
    try:
        ...
        logger.error(f"Failed to run evolution engine: {e}")
        raise HTTPException(status_code=500, detail=str(e))
